# basillians.github.io

Portfolio for a Senior/Principal ML Engineer or AI Research Engineer. Built with Next.js, Tailwind CSS, and file-based MDX content.

## Add content

Create one `.mdx` file in `content/projects`, `content/research`, or `content/writing`. Copy the frontmatter shape from an existing entry; the site will automatically add it to the right archive and generate its detail page.

## Development

```bash
npm install
npm run dev
```

Deploy by importing this repository into Vercel. Every push to the production branch will trigger a deployment.
